appcan.define("slider", function($, exports, module) {
    var model_item = '<div class="slider-item <%=data.id%>" style="background-image:url(<%=data.img%>)">\
    <span class="uabs"><%=data.label%></span>\
    </div>';
    var model_Conitem='<div class="slider-item1" style="position:relative;display:inline-block;font-size:1em;" id="<%=option.index%>">\
    ...
    SliderView.prototype = {
        ...
        set:function(data){
            ...
            self.container = self.container || $('<div class="slider-group"></div>');
